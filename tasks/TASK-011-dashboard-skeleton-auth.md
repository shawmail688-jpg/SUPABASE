...
审查结论：本地 Review PASS。`web/dashboard/` 文件齐全；`#/selftest` 9/9（含 null 坐标防假 Pin）；无 `tile.openstreetmap.org`；config 注入不含 secret；leader/work 使用同一 API 读取契约，surveyor 被挡在 field-form 入口；原占位地图已替换为真实 Leaflet/Esri 地图。真实登录与领导手机视觉验收留 TASK-013/015。
