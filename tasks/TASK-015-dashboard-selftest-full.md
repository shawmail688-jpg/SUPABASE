...
状态：IN REVIEW（2026-09-15：core selftest 11/11、full selftest 8/8，真实地图/照片/风水卡桌面视觉走查通过）
...
审查结论：本地 Review PASS。manager/surveyor/admin 的按钮矩阵、历史保留、编辑字段和 CSV 契约均有自测；新增断言覆盖无坐标文案、Leaflet 地图模块、照片缩略图、风水卡及 hidden 店面不进入 Atlas。真实地图已完成本地桌面走查，列表定位、照片 popup/灯箱、风水详情、Google Maps 外链与图层切换正常。真实 RPC 越权、Realtime ≤3s、375px 全量及用户视觉确认待 TASK-013 账号窗口。
