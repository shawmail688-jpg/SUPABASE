(function () {
  ...
  function runSelfTest(){var checks=[];function ck(n,o){checks.push((o?'PASS ':'FAIL ')+n);}var r=api.latestSurvey(demoSites[0]);ck('latest-survey-prefers-surveyed-at',r&&r.rent===3960);ck('missing-location-labelled-clearly',locationLabel(demoSites[1])==='No location submitted');ck('null-coordinates-never-map',!window.DashboardMap.isMappable({status:'candidate',lat:null,lon:null}));ck('photo-thumbnail-rendered',photoStrip(demoSites[0],1,'test-photo',false).indexOf('<img')>=0);ck('fengshui-card-rendered',fengshuiCard(demoSites[0]).indexOf('FENG SHUI')>=0);ck('usd-default',(r.raw.currency||'USD')==='USD');ck('leaflet-map-module-present',!!window.DashboardMap);ck('tile-config-present',!!(window.MAP_TILE_CONFIG&&window.MAP_TILE_CONFIG.errorThreshold===3));ck('no-osm-public-tile',window.MAP_TILE_CONFIG.streetUrl.indexOf('openstreetmap.org')<0);ResilientTileSource.reset();ResilientTileSource.recordError();ResilientTileSource.recordError();ck('tile-failover-waits-for-threshold',ResilientTileSource.mode()==='street');ResilientTileSource.recordError();ck('tile-failover-after-three-errors',ResilientTileSource.mode()==='satellite'&&ResilientTileSource.current()===MAP_TILE_CONFIG.satelliteUrl);var pre=document.createElement('pre');pre.id='selftest-result';pre.textContent='DASHBOARD SELFTEST '+checks.filter(function(x){return x.indexOf('PASS')===0;}).length+'/'+checks.length+'\n'+checks.join('\n');view.innerHTML='';view.appendChild(pre);document.title=checks.every(function(x){return x.indexOf('PASS')===0;})?'DASHBOARD_SELFTEST_ALL_PASS':'DASHBOARD_SELFTEST_FAIL';}
